...
func (router *Router) Save(w http.ResponseWriter, r *http.Request) {
	...
	logger = logger.With("component", "save handler", "reqID", reqID)
	...
	if err != nil {
		...
		router.logger.Logger.Debug("bad request")
	...
	if err != nil {
		if errors.Is(err, storageErrors.ErrAliasAlreadyExists) {
			...
			response := ResponseSave{
				Id:       -1,
				Response: mainresponse.NewError(errors.New("alias already exists").Error()),
			}
